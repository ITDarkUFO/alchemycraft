...
import net.alchemycraft.block.AbstractChestBlock;
...
public class DisappearingChestBlockEntityRenderer<T extends BlockEntity>
    ...
    public DisappearingChestBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
    ...
    public void render(T entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers,
        ...
        DoubleBlockProperties.PropertySource<? extends DisappearingChestBlockEntity> propertySource;
        World world = ((BlockEntity) entity).getWorld();
        boolean bl = world != null;
        BlockState blockState = bl ? ((BlockEntity) entity).getCachedState()
                : (BlockState) BlocksConfig.DISAPPEARING_CHEST.getDefaultState().with(DisappearingChestBlock.FACING,
                        Direction.SOUTH);
        ChestType chestType = blockState.contains(DisappearingChestBlock.CHEST_TYPE)
                ? blockState.get(DisappearingChestBlock.CHEST_TYPE)
                : ChestType.SINGLE;
        ...
        float g = ((Float2FloatFunction) propertySource
                .apply(DisappearingChestBlock.getAnimationProgressRetriever((LidOpenable) entity))).get(tickDelta);
        g = 1.0f - g;
        g = 1.0f - g * g * g;
