...
public class LeadOreGeneratorFeature {
	private static int vein_size = 3, veins_per_chunk = 20, lower_bound = 0;

	private static FeatureConfig OVERWORLD_LEAD_ORE_FEATURE_CONFIG = new OreFeatureConfig(
			OreConfiguredFeatures.STONE_ORE_REPLACEABLES,
			BlocksConfig.LEAD_ORE.getDefaultState(),
			vein_size);

	@SuppressWarnings("all")
	private static ConfiguredFeature<?, ?> OVERWORLD_LEAD_ORE_CONFIGURED_FEATURE = new ConfiguredFeature(
			Feature.ORE,
			OVERWORLD_LEAD_ORE_FEATURE_CONFIG);
	...
	public static void init() {
		Registry.register(BuiltinRegistries.CONFIGURED_FEATURE,
				new Identifier(MOD_ID, "overworld_lead_ore"),
				OVERWORLD_LEAD_ORE_CONFIGURED_FEATURE);
